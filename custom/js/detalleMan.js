...
$(document).ready(function () {
	...
	ruta = "php_action/fetchDetMan.php";
	ruta2= "php_action/deleteDetMan.php";
	envio1 = "manid=" + prueba;
	url = ruta + "?" + envio1;
	url2= ruta2 +"?" +envio1;
...
function editMaq(maqid = null) {
	if (maqid) {
		...
		$.ajax({
			url: 'php_action/fetchSelectedDetMan.php',
			type: 'post',
			data: { maqid: maqid },
			dataType: 'json',
			success: function (response) {
				...
				$("#editNommaqName").val(response.DESCRIPCIONMANTE);
				$("#editKilom").val(response.PRECIOMANTE);

				// add the categories id 
				$(".editMaqFooter").after('<input type="hidden" name="editmaqid" id="editmaqid" value="' + response.DETALLEMANTEID + '" />');
